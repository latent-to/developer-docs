...
Two curves, both normalized to fraction of maximum (0→1):
  - Conviction growth:   f(t) = 1 - exp(-t / 90)   τ = 648,000 blocks ≈ 90 days
  - Unlock availability: f(t) = 1 - exp(-t / 30)   τ = 216,000 blocks ≈ 30 days
...
def conviction(t):
    return 1 - math.exp(-t / 90)

def unlock_access(t):
    return 1 - math.exp(-t / 30)
...
for xd, col in [(30, ORANGE), (90, BLUE)]:
    x = px(xd)
...
dot_r = 4
for (xd, func, col) in [(30, unlock_access, ORANGE), (90, conviction, BLUE)]:
...
l(f'  <rect x="{lx-10}" y="{ly-14}" width="200" height="76" rx="4" fill="white" stroke="{GRID}" stroke-width="1"/>')
...
l(f'  <text x="{lx+28}" y="{ly+21}" font-size="11" fill="{MUTED}" font-family="sans-serif">τ = 648,000 blocks (≈ 90 days)</text>')
...
l(f'  <text x="{lx+28}" y="{ly+58}" font-size="11" fill="{MUTED}" font-family="sans-serif">τ = 216,000 blocks (≈ 30 days)</text>')
