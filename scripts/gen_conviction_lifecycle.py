...
Generate conviction-lifecycle.svg — a single sequential timeline showing:
  ...
  Scenario: lock 100α at day 0, call unlock_stake(50α) at day 90 (≈ 3τ_conv, conviction ≈ 95%).

  Phase 1 (day 0-90):
    - Conviction grows from 0 toward 100α ceiling: c(t) = 100·(1−exp(−t/30))
    - At day 90 (3τ), conviction ≈ 95α

  Unlock event (day 90):
    - locked_mass: 100 → 50
    - conviction:  95 → 45  (drops by 50, the unlocked amount)
    - unlocked_mass: 0 → 50  (enters ~365-day decay period)

  Phase 2 (day 90-500):
    - Conviction rebuilds from 45 toward new ceiling 50:
        c(t) = 50 − (50−45)·exp(−(t−90)/30)
    - Unlocked α becomes available over ~365 days:
        available(t) = 50·(1−exp(−(t−90)/τ_unlock))
...
c_at_unlock   = LOCK_AMOUNT * (1 - math.exp(-UNLOCK_DAY / TAU_CONV))   # ≈ 95
c_after_unlock = c_at_unlock - UNLOCK_AMOUNT                             # ≈ 45
...
l(f'  <rect x="{ML}" y="{MT}" width="{x_unlock-ML:.2f}" height="{PH}" fill="#eff6ff" opacity="0.5"/>')
l(f'  <rect x="{x_unlock:.2f}" y="{MT}" width="{ML+PW-x_unlock:.2f}" height="{PH}" fill="#fff7ed" opacity="0.5"/>')

# ── grid ──────────────────────────────────────────────────────────────────────
for alpha in [25, 50, 75, 100]:
    y = py(alpha)
    l(f'  <line x1="{ML}" y1="{y:.2f}" x2="{ML+PW}" y2="{y:.2f}" stroke="{GRID}" stroke-width="1"/>')
for xd in range(0, X_MAX + 1, 90):
    x = px(xd)
...
l(f'  <text x="{x_unlock+5}" y="{MT+31}" font-size="10" fill="{MUTED}" font-family="sans-serif">day {UNLOCK_DAY} (≈ 3τ)</text>')
...
l(f'  <text x="{xmid:.2f}" y="37" text-anchor="middle" font-size="11" fill="{MUTED}" font-family="sans-serif">Scenario: lock 100α at day 0, call unlock_stake(50α) at day 90 (≈ 3τ)</text>')
...
l(f'  <text x="{x_p2_mid:.2f}" y="{MT+PH-10}" text-anchor="middle" font-size="11" fill="{ORANGE}" font-family="sans-serif" opacity="0.8">Phase 2 — unlock period (~365 days) + conviction rebuilds</text>')

# ── legend ────────────────────────────────────────────────────────────────────
lx, ly = ML + PW - 220, MT + PH - 90
l(f'  <rect x="{lx-8}" y="{ly-13}" width="225" height="72" rx="4" fill="white" stroke="{GRID}" stroke-width="1"/>')
