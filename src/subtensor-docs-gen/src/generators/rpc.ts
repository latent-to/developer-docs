...
export async function generateRpc(
  ...
  if (nodeMethods) {
    console.log(
      `  ℹ  rpc_methods returned ${nodeMethods.size} methods — filtering to node-supported RPCs`,
    );
  } else {
    console.log(
      "  ⚠  rpc_methods unavailable — including all known definitions",
    );
  }
  ...
  const allNamespaces = new Map<
    string,
    Map<string, { info: MethodInfo; def: any }>
  >();
  ...
  for (const [n] of sortedNs) lines.push(`- **[${n}](#${palletAnchor(n)})**`);
  ...
  for (const [nsName, methods] of sortedNs) {
    ...
    for (const m of methods) {
      ...
      if (m.isUnsafe)
        lines.push(`- **unsafe**: this method is flagged as unsafe`);
