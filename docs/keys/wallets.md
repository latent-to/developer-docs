...
For working with keys programmatically, the SDK exposes the `Keypair` and `Wallet` classes from the [Bittensor Wallet SDK](./btwallet/index.md), which handles key generation, signing, and asymmetric message encryption.
