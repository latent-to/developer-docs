...
- [SDK Tutorials](index): Overview of SDK procedural content
- [Staking with a Proxy](../../keys/proxies/staking-with-proxy): Practical tutorial using `GenericCall` with proxies
- [Managing Subtensor Connections](managing-subtensor-connections): Connection management for sync and async operations
- [Async Operations](../subnets/asyncio): Working with AsyncSubtensor

## Introduction

In the Bittensor ecosystem, many functions accept a `call` argument of type `GenericCall`. This allows you to create blockchain calls separately from executing them, enabling advanced use cases like proxies, crowdloans, MEV protection, and fee estimation.
...
## Historical Context: Creating Calls
...
# Can even wrap in Sudo (if you have permissions)
sudo_call = Sudo(subtensor).sudo(call=proxy_call)
```
